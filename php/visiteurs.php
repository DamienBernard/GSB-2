...
<html>
    ...
    <body> <!-- Corps du document -->
        ...
        <div class="right">
            ...
            <h1 id="titrePage">INFORMATIONS | VISITEURS</h1>
            ...
            <div id="informations" class="container col-6">
                <?php
                    ...
                    // gestion de l'évènement sur la liste deroulante listeEtud
                    echo '<select id="listeDpt" onchange="javascript:envoyerRequeteListeDepartement(this.value)">';
                    ...
                    if ($ligne!=null)
                    {
                        echo "<option selected value='00'>--- Choix du Département ---";

                        //$ligne = $leslignes->fetch();
                        while($ligne!=null)
                            {
                            echo '<option value='.$ligne['code'].'>'.$ligne['nom'];
                            $ligne = $leslignes->fetch();
                            }
                    }
                    else
                    ...
                    echo '</select>';
                    //----------------------------------------------------------------------------------------//
                    echo '<br><br><span id="labelListeVisit" style="visibility:hidden">Liste des Visiteurs : </span><br>';
                    // gestion de l'évènement sur la liste deroulante listeEtud
                    echo '<select id="listeVisit"  style="visibility:hidden" onchange="javascript:envoyerRequeteVisiteur(this.value)">';

                    $sql2='select * from VISITEUR order by VIS_NOM';
                    $leslignes=$conn->query($sql2) ;
                    $ligne = $leslignes->fetch();
                    if ($ligne!=null)
                    {
                        echo '<option selected value='.$ligne['VIS_MATRICULE'].'>'.$ligne['VIS_NOM']."  ".$ligne['Vis_PRENOM'];
                        $matricule=$ligne['VIS_MATRICULE'];
                        $nom=$ligne['VIS_NOM'];
                        $prenom=$ligne['Vis_PRENOM'];
                        $adresse=$ligne['VIS_ADRESSE'];
                        $ville=$ligne['VIS_VILLE'];
                        $secteur=$ligne['SEC_CODE'];
                        $ligne = $leslignes->fetch();
                        while($ligne!=null)
                            {
                            echo '<option value='.$ligne['VIS_MATRICULE'].'>'.$ligne['VIS_NOM']."  ".$ligne['Vis_PRENOM'];
                            $ligne = $leslignes->fetch();
                            }
                    }
                    else
                    ...
                    echo '<br><span id="infosNom" style="visibility:hidden"><u>NOM</u> :</span> <span id="nom" style="visibility:hidden">'.$nom.'</span>';
                    echo '<br><span id="infosPrenom" style="visibility:hidden"><u>PRÉNOM</u> :</span> <span id="prenom" style="visibility:hidden">'.$prenom.'</span>';
                    echo '<br><span id="infosAdresse" style="visibility:hidden"><u>ADRESSE</u> :</span> <span id="adresse" style="visibility:hidden">'.$adresse.'</span>';
                    echo '<br><span id="infosVille" style="visibility:hidden"><u>VILLE</u> :</span> <span id="ville" style="visibility:hidden">'.$ville.'</span>';
                    echo '<br><span id="infosSecteur" style="visibility:hidden"><u>SECTEUR</u> :</span> <span id="secteur" style="visibility:hidden">'.$secteur.'</span>';


                ?>
                <br><br>
                <div id="prnx" class="btn-group">
                    <input id="btnPrecedent" type="button" class="btn btn-secondary btn-sm" value="Précédent" onclick="visiteurPrecedent()" style="visibility:hidden">
                    <input id="btnSuivant" type="button" class="btn btn-secondary btn-sm" value="Suivant" onclick="visiteurSuivant()" style="visibility:hidden">
                </div>
            </div>
        </div>
    </body>
