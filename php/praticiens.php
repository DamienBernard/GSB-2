...
<html>
    ...
    <body <!-- Corps du document -->
        ...
        <div class="right">
            ...
            <h1 id="titrePage">INFORMATIONS | PRATICIENS</h1>
            ...
            <div id="informations" class="container col-6">
                ...
                <?php
                    try{
                        echo "<select id='listeDeroulantePraticiens' onchange='javascript:envoyerRequetePraticien(this.value)'>";
                        ...
                        if($donnees != null){
                            $numero = $donnees['PRA_NUM'];
                            $nom = $donnees['PRA_NOM'];
                            $prenom = $donnees['PRA_PRENOM'];
                            $adresse = $donnees['PRA_ADRESSE']." ".$donnees['PRA_VILLE'];
                            $cp = $donnees['PRA_CP'];
                            $coef = $donnees['PRA_COEFNOTORIETE'];
                            $type = $donnees['TYP_CODE'];

                            echo "<option selected value=".$donnees['PRA_NUM'].">".$donnees['PRA_NOM']." ".$donnees['PRA_PRENOM'];
                            while($donnees = $reponse->fetch()){
                                $unPraticien = "<option value='".$donnees['PRA_NUM']."'>".$donnees['PRA_NOM']." ".$donnees['PRA_PRENOM']."</option>";
                                echo $unPraticien;
                            }
                        ...
                        $value = $bdd->query('SELECT COUNT(*) FROM PRATICIEN');
                        $valueMax = $value->fetch();

                        echo "</select>";
                        echo "</p>";
                        echo "<p><u>NUMÉRO</u> : <span id='num'>".$numero."</span></p>";
                        echo "<p><u>NOM</u> : <span id='nom'>".$nom."</span></p>";
                        echo "<p><u>PRÉNOM</u> : <span id='prenom'>".$prenom."</span></p>";
                        echo "<p><u>ADRESSE</u> : <span id='adresse'>".$adresse."</span></p>";
                        echo "<p><u>CODE POSTAL</u> : <span id='cp'>".$cp."</span></p>";
                        echo "<p><u>COEFFICIENT DE NOTORIÉTÉ</u> : <span id='coef'>".$coef."</span></p>";
                        echo "<p><u>TYPE</u> : <span id='type'>".$type. "</span></p>";
                ...
                <div id="prnx" class="btn-group">
                    <button type="button" class="btn btn-secondary btn-sm" onclick="javascript:praticienPrecedent()">Précédent</button>
                    <?php echo "<button type='button' class='btn btn-secondary btn-sm' onclick='javascript:PraticienSuivant(".$valueMax[0].")'>Suivant</button>"; ?>
                </div>
            </div>
        </div>
    </body>
