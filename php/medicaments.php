...
<body <!-- Corps du document -->
    ...
    <div id="MED" class="right">
        <img src='../images/banniere.png' id="logoConnecte" height="200" alt="">
        <!-- DAMIEN - Il faut que tu modifies que dans cette <div>-->

        <div class="container">
            
            <h1 id="titrePage">INFORMATIONS | MÉDICAMENTS</h1>
            
            <hr class="col-9">
            
            <div id="informations" class="container col-6">
            <?php
                set_exception_handler('gestion_erreur');

                try {
                    include('connexion.inc.php');
                    echo '<span>Liste Médicaments : </span>';
                    echo '<select id="listeMed" name="listeMedicaments" onchange="javascript:envoyerRequeteMedicament(this.value)">';
                    // requete sql qui renvoie tous les MED_DEPOTLEGAL  de ma table medicament
                    $sql="select * from MEDICAMENT order by MED_DEPOTLEGAL asc";
                    // execute la requete mise en parametre
                    $lesMedicaments=$conn->query($sql);
                    // recupere le premier enregistrer du renvoie de la requete
                    $medicament=$lesMedicaments->fetch();

                    // construction de la liste deroulante
                    if ($medicament!=null) {
                        $libelleMedDepot=$medicament['MED_DEPOTLEGAL'];
                        $libelleComm=$medicament['MED_NOMCOMMERCIAL'];
                        $libelleFam=$medicament['FAM_CODE'];
                        $libelleCompo=$medicament['MED_COMPOSITION'];
                        $libelleEffets=$medicament['MED_EFFETS'];
                        $libelleContreIndic=$medicament['MED_CONTREINDIC'];
                        $libellePrix=$medicament['MED_PRIXECHANTILLON'];
                        echo '<option selected value='.$medicament["MED_DEPOTLEGAL"].'>'.$medicament["MED_NOMCOMMERCIAL"];
                        $medicament=$lesMedicaments->fetch();
                        while ($medicament!=null) {
                            echo '<option value='.$medicament["MED_DEPOTLEGAL"].'>'.$medicament["MED_NOMCOMMERCIAL"];
                            $medicament=$lesMedicaments->fetch();
                        }
                    }
                    else {
                    ...
                    echo "</select>";
                    echo "<br /><br />";
                    // bloque affiche les informations du medicament
                    echo "<p id='infomed'>";
                    echo "<u>DÉPOT LÉGAL</u> : <span class='info' id='MED_DEPOTLEGAL'>".$libelleMedDepot."</span><br /><br />";
                    echo "<u>FAMILLE</u> : <span class='info' id='FAM_CODE'>".$libelleFam."</span><br /><br />";
                    echo "<u>LIBELLÉ COMMERCIAL</u> : <span class='info' id='MED_NOMCOMMERCIAL'>".$libelleComm."</span><br /><br />";
                    echo "<u>COMPOSITION DU MÉDICAMENT</u> : <span class='info' id='MED_COMPOSITION'>".$libelleCompo."</span><br /><br />";
                    echo "<u>EFFET DU MÉDICAMENT</u> : <span class='info' id='MED_EFFETS'>".$libelleEffets."</span><br /><br />";
                    echo "<u>CONTRE INDIQUE</u> : <span class='info' id='MED_CONTREINDIC'>".$libelleContreIndic."</span><br /><br />";
                    echo "<u>PRIX DU MÉDICAMENT</u> : <span class='info' id='MED_PRIXECHANTILLON'>".$libellePrix."</span><br /><br /></p>";
             ...
             <div id="prnx" class="btn-group" role="group" aria-label="Basic example">
                 <button type="button" class="btn btn-secondary" onclick="selectPrecedent()">Précédent</button>
                 <button type="button" class="btn btn-secondary" onclick="selectSuivante()">Suivant</button>
             </div>
        </div>
    </div>
</body>
